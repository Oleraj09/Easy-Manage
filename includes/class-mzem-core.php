...
class MZEM_Core
{
    ...
    public function enqueue_assets($hook)
    {
        ...
        // Always register SortableJS so it can be declared as a dependency.
        // It will only be enqueued (output to page) on the Custom Order page.
        $page = filter_input(INPUT_GET, 'page', FILTER_SANITIZE_SPECIAL_CHARS);
        ...
        // On the Custom Order page, make sortable-js a hard dependency of the
        // admin script so the browser always executes Sortable before our code.
        $admin_deps = array('jquery');
        if ('mzem-custom-order' === $page) {
            wp_enqueue_script('sortable-js');
            $admin_deps[] = 'sortable-js';
        }
